...
// Internal query for getting session user - avoids circular dependency with api.user
export const internalGetSessionUser = internalQuery({
  args: {},
  handler: async (ctx: QueryCtx) => {
    const table = entsTableFactory(ctx, entDefinitions);
    const session = await getSession({ ...ctx, table } as any);
    ...
    const [user, subscription] = await Promise.all([
      table('user').get(session.userId as Id<'user'>),
      (async () => {
        if (!activeOrganizationId) {
          return null;
        }
