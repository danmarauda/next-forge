...
const DEFAULT_CONFIG: SetupConfig = {
  ...
  webhookUrl:
    ...
    'http://localhost:3002/api/webhooks/workos',
};
...
async function setupSSO(
  ...
  try {
    ...
    console.log(
      `    https://dashboard.workos.com/organizations/${organizationId}/sso`,
    );
    console.log(`  Redirect URI: ${config.redirectUri}`);
  } catch (error) {
    throw new Error(
      `Failed to setup SSO: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
}
...
async function setupDirectorySync(
  ...
  try {
    ...
    if (directories.data.length > 0) {
      console.log(
        `  Found ${directories.data.length} existing directory sync connection(s)`,
      );
      ...
      console.log(
        `    https://dashboard.workos.com/organizations/${organizationId}/directory-sync`,
      );
    }
  } catch (error) {
    throw new Error(
      `Failed to setup Directory Sync: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
}
