...
function recordResult(feature: string, success: boolean, message: string, details?: any) {
...
async function testAPIConnection(workos: WorkOS) {
  try {
    ...
    recordResult("API Connection", true, "Successfully connected to WorkOS API");
  } catch (error) {
    recordResult("API Connection", false, `Failed to connect: ${error instanceof Error ? error.message : "Unknown error"}`);
    throw error;
  }
}
...
async function testOrganizations(workos: WorkOS) {
  try {
    ...
    recordResult("Organizations", true, `Found ${orgs.data.length} organization(s)`);
    ...
    recordResult("Organizations", false, `Failed to list organizations: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
...
async function testUsers(workos: WorkOS) {
  try {
    ...
    recordResult("Users", false, `Failed to list users: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
...
async function testAuthenticationMethods(workos: WorkOS) {
  try {
    ...
    const redirectUri = process.env.WORKOS_REDIRECT_URI || "https://ara.aliaslabs.ai/auth/callback";
    const authUrl = await workos.userManagement.getAuthorizationUrl({
      clientId: process.env.WORKOS_CLIENT_ID!,
      redirectUri,
    });
    
    recordResult("Authentication Methods", true, "Authorization URL generated successfully");
    console.log(`    └─ Auth URL: ${authUrl.substring(0, 80)}...`);
  } catch (error) {
    recordResult("Authentication Methods", false, `Failed to generate auth URL: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
  ...
  try {
    ...
    if (orgs.data.length > 0) {
      ...
      try {
        ...
        recordResult("SSO", true, "SSO authorization URL generated successfully");
    ...
    recordResult("SSO", false, `SSO test failed: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
...
async function testDirectorySync(workos: WorkOS) {
  try {
    ...
    recordResult("Directory Sync", true, `Found ${directories.data.length} directory sync connection(s)`);
    ...
    if (directories.data.length > 0) {
      ...
      console.log(`    └─ Note: Configure Directory Sync connections in dashboard`);
    }
  } catch (error) {
    recordResult("Directory Sync", false, `Failed to list directories: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
...
async function testAuditLogs(workos: WorkOS) {
  try {
    ...
    recordResult("Audit Logs", true, `Found ${events.data.length} audit log event(s)`);
    ...
    recordResult("Audit Logs", false, `Failed to list audit logs: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
...
async function testAdminPortal(workos: WorkOS) {
  try {
    ...
    if (orgs.data.length > 0) {
      ...
      recordResult("Admin Portal", true, "Admin portal link generated successfully");
      console.log(`    └─ Portal Link: ${link.substring(0, 80)}...`);
    } else {
      recordResult("Admin Portal", true, "Admin Portal ready (no organizations found)");
    }
  } catch (error) {
    recordResult("Admin Portal", false, `Failed to generate portal link: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}

async function testWebhookEndpoint() {
  try {
    const webhookUrl = process.env.WORKOS_WEBHOOK_URL || "https://api.ara-group.com/api/webhooks/workos";
    ...
    if (webhookUrl.includes("localhost") || webhookUrl.includes("127.0.0.1")) {
      try {
        ...
        recordResult("Webhook Endpoint", true, `Webhook endpoint is accessible (${response.status})`);
      } catch (error) {
        recordResult("Webhook Endpoint", false, `Webhook endpoint not accessible: ${error instanceof Error ? error.message : "Unknown error"}`);
        console.log(`    └─ Note: Start the development server to test webhooks`);
      }
    } else {
      recordResult("Webhook Endpoint", true, `Webhook URL configured: ${webhookUrl}`);
      console.log(`    └─ Note: Test webhooks from WorkOS dashboard`);
    }
  } catch (error) {
    recordResult("Webhook Endpoint", false, `Webhook test failed: ${error instanceof Error ? error.message : "Unknown error"}`);
  }
}
