...
async function getOrCreateOrganization(workos: WorkOS) {
  try {
    ...
    const araOrg = orgs.data.find((org) => org.name === 'ARA Group Platform');
    ...
    throw new Error(
      `Failed to get/create organization: ${error instanceof Error ? error.message : 'Unknown error'}`,
    );
  }
}
...
async function createOrGetSuperAdmin(
  ...
  try {
    ...
    if (existingUsers.data.length > 0) {
      ...
      console.log(
        `  ✅ Found existing user: ${admin.firstName} ${admin.lastName} (${user.email}) - ${admin.type}`,
      );
    } else {
      // Create new user
      ...
      console.log(
        `  ✅ Created user: ${admin.firstName} ${admin.lastName} (${user.email}) - ${admin.type}`,
      );
    }
    ...
    if (memberships.data.length === 0) {
      ...
      console.log(
        `  ✅ Added ${admin.firstName} ${admin.lastName} to organization`,
      );
    }
    ...
    console.error(
      `  ❌ Failed to create/get user ${admin.firstName} ${admin.lastName}:`,
      error,
    );
    throw error;
  }
...
async function assignSuperAdminRole(
  ...
  try {
    ...
    console.error(
      `  ⚠️  Role assignment may need to be done in dashboard:`,
      error,
    );
  }
}
...
function printSummary(organizationId: string) {
  ...
  results.forEach((result) => {
    ...
    if (result.userId) {
      ...
      console.log(
        `     Dashboard: https://dashboard.workos.com/environment_01K5K3Y79TXRCBAA52TCYZ5CCA/users/${result.userId}`,
      );
    }
  });
  ...
  console.log(
    `     https://dashboard.workos.com/environment_01K5K3Y79TXRCBAA52TCYZ5CCA/organizations/${organizationId}`,
  );
