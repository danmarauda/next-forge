...
async function main() {
  ...
  if (!WORKOS_API_KEY) {
    ...
    console.log("   1. Get your API key from: https://dashboard.workos.com/api-keys");
  ...
  try {
    ...
    if (!existsSync(join(convexPath, "convex.json"))) {
      ...
      if (WORKOS_API_KEY) {
        try {
          runCommand(`convex env set WORKOS_API_KEY "${WORKOS_API_KEY}"`, convexPath);
          console.log("  ✅ WORKOS_API_KEY set in Convex");
        } catch (error) {
          console.log("  ⚠️  Could not set WORKOS_API_KEY in Convex (may already be set)");
        }
      }
      ...
      if (WORKOS_CLIENT_ID) {
        try {
          runCommand(`convex env set WORKOS_CLIENT_ID "${WORKOS_CLIENT_ID}"`, convexPath);
