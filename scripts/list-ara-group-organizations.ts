...
function main() {
  ...
  ARA_GROUP_ORGANIZATIONS.forEach((org, index) => {
    ...
    if (org.productionDomains && org.productionDomains.length > 0) {
      console.log(
        `   Production Domains: ${org.productionDomains.join(', ')} (future)`,
      );
    }
  ...
  const productionDomains = ARA_GROUP_ORGANIZATIONS.flatMap(
    (org) => org.productionDomains || [],
  );
  ...
  console.log(
    '   - All organizations should be created as separate WorkOS organizations',
  );
