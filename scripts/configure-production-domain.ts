...
const PROJECT_ID = process.env.VERCEL_PROJECT_ID || 'prj_zm0h3A5Xcs82mN5ArivZ8Gm4V4I5';
...
async function findProjectWithDomain(): Promise<string | null> {
  ...
  try {
    ...
    for (const project of projects) {
      try {
        const domains = await apiCall(`/v9/projects/${project.id}/domains?teamId=${TEAM_ID}`);
        const hasDomain = domains.domains?.some((d: Domain) => d.name === DOMAIN);
        
        if (hasDomain) {
          console.log(`✓ Found ${DOMAIN} in project: ${project.name} (${project.id})`);
          return project.id;
        }
      } catch (err) {
