...
async function main() {
  ...
  if (!apiKey) {
    console.log("⚠️  WORKOS_API_KEY not set - running tests that don't require API");
    console.log("   Set WORKOS_API_KEY in apps/app/.env.local to run full tests\n");
  }
  ...
  try {
    ...
    console.error("\n❌ Testing failed:", error instanceof Error ? error.message : error);
    process.exit(1);
  }
}
...
function testEnvironmentConfiguration() {
  ...
  optional.forEach((varName) => {
    if (process.env[varName]) {
      recordTest("Environment", varName, "pass", `${varName} is set (optional)`);
    } else {
      recordTest("Environment", varName, "warning", `${varName} not set (optional)`);
    }
  });
}
...
async function testAPIConnection(workos: WorkOS) {
  try {
    ...
    recordTest("API", "Connection", "pass", "Successfully connected to WorkOS API");
  } catch (error) {
    recordTest("API", "Connection", "fail", `Failed to connect: ${error instanceof Error ? error.message : "Unknown"}`);
  }
}
...
async function testCoreFeatures(workos: WorkOS) {
  try {
    ...
    recordTest("Organizations", "List", "pass", `Found ${orgs.data.length} organization(s)`);
    ...
    recordTest("Authentication", "Authorization URL", "pass", "Authorization URL generated successfully");

  } catch (error) {
    recordTest("Core Features", "Tests", "fail", `Failed: ${error instanceof Error ? error.message : "Unknown"}`);
  }
}
...
function testWebhookConfiguration() {
  ...
  if (webhookUrl) {
    if (webhookUrl.startsWith("https://")) {
      recordTest("Webhooks", "Configuration", "pass", `Webhook URL configured: ${webhookUrl}`);
    } else {
      recordTest("Webhooks", "HTTPS", "warning", `Webhook uses HTTP: ${webhookUrl} (use HTTPS in production)`);
    }
    ...
    try {
      ...
      if (webhookHandler.includes("handleWebhook")) {
        ...
        recordTest("Webhooks", "Handler", "warning", "Webhook handler may be incomplete");
      }
    } catch (error) {
      recordTest("Webhooks", "Handler", "fail", "Webhook handler not found");
    }
  } else {
    recordTest("Webhooks", "Configuration", "warning", "Webhook URL not configured");
  }
}
...
function testCodeIntegration() {
  ...
  try {
    ...
    recordTest("Code", "WorkOS Service", "pass", "WorkOS service package exists");
  } catch (error) {
    recordTest("Code", "WorkOS Service", "fail", "WorkOS service package not found");
  }
  ...
  try {
    ...
    if (convexWorkos.includes("handleWebhook")) {
      recordTest("Code", "Convex Integration", "pass", "Convex WorkOS integration exists");
    } else {
      recordTest("Code", "Convex Integration", "warning", "Convex integration may be incomplete");
    }
  } catch (error) {
    recordTest("Code", "Convex Integration", "fail", "Convex WorkOS integration not found");
  }
  ...
  try {
    ...
    if (featureFlags.includes("workos")) {
      recordTest("Code", "Feature Flags", "pass", "WorkOS feature flags configured");
    } else {
      recordTest("Code", "Feature Flags", "warning", "WorkOS feature flags may be incomplete");
    }
  } catch (error) {
    recordTest("Code", "Feature Flags", "fail", "Feature flags package not found");
  }
}
